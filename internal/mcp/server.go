...
// NewHandler builds an MCP server with ping, server_info, execute_command, and upload_file tools,
...
//   - uplCfg: configuration for the upload_file tool (file-upload task).
//
// Returns an error only if the server cannot be constructed (fatal for serve).
// Does not panic.
//
// Contract (plan.md §Contracts):
//   mcp.NewHandler(ver string, audit server.AuditFn, execCfg cmdexec.Config, uplCfg fileupload.Config) (http.Handler, error)
// SR-28: no second auth channel; SR-29: same port/TLS as transport (mounted by caller).
// ADR-004: execute_command and upload_file are NOT wrapped with withAudit — they own their own audit.
func NewHandler(ver string, audit server.AuditFn, execCfg cmdexec.Config, uplCfg fileupload.Config) (http.Handler, error) {
	...
	// - upload_file: NOT wrapped with withAudit (ADR-004-стиль/SR-78);
	//   uploadHandler writes its own upload-audit record in all branches.
	...
	sdkmcp.AddTool(s, uploadTool(), uploadHandler(uplCfg, audit))
