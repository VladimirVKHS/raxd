...
6. **Единый тон с bootstrap.** Ошибки: `error:` + `hint:`. Таблица `key list`: полная Unicode-рамка
   (стандартный вывод `olekukonko/tablewriter`). Всё в одном стиле с `raxd status` и `raxd version`.
...
**Стиль таблицы:** полная Unicode-рамка — стандартный вывод `olekukonko/tablewriter` v1.x без
дополнительной настройки стиля. Ранее предложенный dash-стиль (и иллюстрация в bootstrap ux-spec
в разделе заглушки `key list`) заменяется этим форматом. Box-drawing — Unicode, не ANSI;
сохраняется при `NO_COLOR` и при пайпинге.

**Канонический макет (есть ключи):**
...
> **Q5-фикс (2026-05-21):** колонка ID теперь показывает полный 16-hex id (было: первые 12 символов).
> Это позволяет напрямую скопировать id из `key list` и передать в `raxd key delete <id>` без
> дополнительных шагов. Макет снят с реального бинаря в Docker — байт-в-байт.

**Ширина колонок:**

| Колонка   | Ширина контента         | Правило усечения                       |
|-----------|-------------------------|----------------------------------------|
| ID        | 16 сим.                 | Полный 16-hex id, без усечения и `…`  |
| LABEL     | динамически по контенту | tablewriter подбирает ширину по самому длинному label; усечение с суффиксом `…` при превышении 20 рун. Ширина в приведённом примере: 14 сим. (`production-key`) |
| CREATED   | 10 сим.                 | Формат `YYYY-MM-DD`                    |
| LAST USED | 9 сим.                  | `YYYY-MM-DD` или `never`               |

Итого с рамкой и отступами: ~62 символа (ID 18 + LABEL 16 + CREATED 12 + LAST USED 11 + 5 границ).

**Дополнительные правила таблицы:**

- ID — ПОЛНЫЙ 16-hex id записи (D5: 8 байт `crypto/rand` → 16 hex-символов); показывается целиком,
  без усечения и без `…`. Значение напрямую годится для `raxd key delete <id>` (exact-match).
...
- Таблица печатается БЕЗ левого отступа (флаш слева от `┌`): `olekukonko/tablewriter` стрипит
  ведущие пробелы. Двухпробельный отступ слева присутствует только в блоке метаданных `key create`
  (это `printf`, не tablewriter).
...
**Пустое хранилище** (таблица не печатается, только текст):
...
**Реализуется через `olekukonko/tablewriter`** со стандартными настройками рамки (полная
Unicode-рамка рендерится по умолчанию в v1.x). Дополнительная стилизация не требуется.
...
2. **`key list` не раскрывает секрет ни при каких условиях.**
   Вывод содержит только: id (полный 16-hex), label (или `-`), created (`YYYY-MM-DD`),
   last-used (`YYYY-MM-DD` или `never`). Хэш, salt, fingerprint, статус `revoked` — не выводятся.

3. **Ошибки оперируют id, label, fingerprint — не телом ключа.**
   Текст ошибки может включать id (случайный идентификатор). Тело `rax_live_…` — никогда.
...
6. **ID в выводе — безопасен.** ID — случайный идентификатор записи, не производный от секрета
   (D5, SR-5). Его показ в ошибках, подтверждениях и таблице не раскрывает ключ. Полный 16-hex id
   в `key list` — это удобство для пользователя (`key delete` принимает exact-match), а не
...
| Border        | `#3A3A3A` (dark) | Рамки баннера, box вокруг ключа и таблицы  |
...
Unicode box-drawing (`┌─┬─┐ │ ├─┼─┤ └─┴─┘`) — не ANSI, не отключается при `NO_COLOR`.
Таблица `key list` и рамка вокруг ключа сохраняются в полном виде при любом значении `NO_COLOR`.
...
- Рамка таблицы `key list` сохраняется (Unicode, не ANSI)
...
Таблица с полной рамкой занимает ~62 символа (ID 18 + LABEL 16 + CREATED 12 + LAST USED 11 +
5 границ). При ширине терминала < 62 символов:
- Сначала усекается `LABEL` (с суффиксом `…`, до 20 рун по правилу усечения)
- При дальнейшем сужении — `olekukonko/tablewriter` адаптирует ширины колонок автоматически
- ID (16 hex) не усекается: потеря символов сделает id непригодным для `key delete`

Заголовки не усекаются.
...
- `raxd key list | grep d7bc3a34da19d94e` — работает корректно (stdout = таблица).
- `raxd key list | less` — корректно, таблица читаема постранично.
- `raxd key delete d7bc3a34da19d94e 2>/dev/null` — на stdout ничего нет; выход по exit-коду.
