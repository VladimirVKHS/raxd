...
> (baseline §6).
>
> **Терминология (важно для tls/mcp).** «Полный ключ» = строка `rax_live_<base64url>` целиком — то,
> что выдаётся пользователю при `create`, передаётся в заголовке `Authorization: Bearer` и подаётся
> в `Verify`. «Тело ключа» = случайная base64url-часть после префикса `rax_live_` (источник энтропии).
> **Хэшируется и сравнивается ПОЛНЫЙ ключ:** хранимый `hash = sha256(полный_ключ ‖ per-key-salt)`,
> `create` и `Verify` согласованно используют полный ключ целиком (а не только base64url-часть).
> Безопасность §1 соблюдена: энтропия base64url-части ≥256 бит, префикс `rax_live_` константный и
> не снижает непредсказуемость. Открытый (полный) ключ в хранилище не лежит — только его хэш+соль.

## Генерация ключей (baseline §1)

- [ ] **SR-1. Тело ключа генерируется только `crypto/rand`, ≥128 бит.** В `internal/keystore`
  (`crypto.go`, `generateBody`) тело (base64url-часть после префикса `rax_live_`) — ≥16 байт из
  `crypto/rand` (план: 32 байта). Проверка: тест на длину/энтропию тела; grep по пакету
  `internal/keystore` НЕ находит импорт `math/rand`. (baseline §1 «Генерация: crypto/rand, ≥128 бит;
  никогда math/rand»; plan: тело 32 байта crypto/rand → D1)
...
- [ ] **SR-6. Формат ключа `rax_live_<base64url>` без padding.** Тело (источник энтропии) кодируется
  `base64.RawURLEncoding`; полный ключ = константный префикс `rax_live_` + base64url-тело. Именно
  полный ключ (целиком, с префиксом) выдаётся при `create`, передаётся в `Authorization: Bearer` и
  подаётся в `Verify`. Проверка: тест на префикс и на отсутствие padding `=`. (baseline §1 «префикс +
  base64url/hex тела»; spec D1)

- [ ] **SR-7. В `keys.db` НЕ хранится открытый ключ — только `sha256(полный_ключ ‖ per-key-salt)` и
  salt.** Хэшируемое значение — ПОЛНЫЙ предъявляемый ключ `rax_live_<base64url>` целиком (то, что
  передаётся и верифицируется), а НЕ только base64url-часть. Сериализуемый `Database`/`Record`
  содержит `hash`=`sha256(полный_ключ ‖ per-key-salt)` (конкатенация полного ключа и соли per
  baseline §1) и `salt`; полный/открытый ключ и `PlainKey` НЕ сериализуются. Поля `hash`/`salt` —
  неэкспортируемые, в JSON только через явные теги; `PlainKey` не входит в `Record`/`Database`.
  Проверка: тест — подстрока ВЫПУЩЕННОГО полного ключа (а равно его base64url-части) отсутствует в
  байтах файла `keys.db` (как в spec AC). (baseline §1 «хранить sha256(key + per-key-salt) + salt,
  не открытый ключ»; plan Contracts/Типы)

- [ ] **SR-8. Хэш считается по схеме baseline (полный ключ ‖ salt), SHA-256.** `hashKey` =
  `sha256(полный_ключ ‖ salt)`, где «полный ключ» — предъявляемая строка `rax_live_<base64url>`
  целиком; `create` (хэширование при выпуске) и `Verify` (хэширование предъявленного) ОБЯЗАНЫ
  использовать одно и то же полное значение (согласованность подтверждается тестом
  `TestHashSchemeDirectVerification`). bcrypt/argon2 НЕ применяются (избыточны при ≥128 бит энтропии
  base64url-части, research Q3). Проверка: инспекция `crypto.go` — конкатенация полного ключа и соли
  перед `sha256`; тест воспроизводит хэш по сохранённым (полному ключу)+соли. (baseline §1)

## Сравнение секретов (baseline §1)

- [ ] **SR-9. Сравнение хэшей/секретов ТОЛЬКО constant-time.** В `Verify` предъявленный ПОЛНЫЙ ключ
  хэшируется с per-key-salt записи и сравнивается с хранимым `hash`; каждое сравнение —
...
- [ ] **SR-10. `==`/`EqualFold`/`bytes.Equal` по секретам/хэшам отсутствуют.** Запрет сравнения
  полного/открытого ключа, хэша или соли через `==`, `strings.EqualFold`, `bytes.Equal`. Проверка:
  grep по `internal/keystore` — нет сравнений секретных/хэш-значений небезопасными операторами; ревью
...
- [ ] **SR-15. fingerprint не раскрывает ключ.** `Fingerprint(presented)` = короткий префикс
  `sha256(полный_ключ)` в hex (8-12 симв., без соли) — для аудита/идентификации, не позволяет
  восстановить ключ при ≥128 бит энтропии. Проверка: тест — fingerprint детерминирован для одного
  полного ключа, его длина ≤12 симв., он не равен телу/полному хэшу. (baseline §1/§4; plan Fingerprint)
...
- [ ] **SR-21. temp-файлы не «текут» с материалом ключа.** В temp пишется только `Database` (хэш+соль,
  без полного ключа/`PlainKey`); при ошибке записи temp удаляется (не остаётся на диске). Проверка:
  тест — после смоделированной ошибки записи temp-файлы в каталоге отсутствуют; полный ключ в temp не
...
- [ ] **SR-24. `create` и `delete` порождают аудит-запись без тела ключа.** Структурная запись
  ...
  stderr демона/journald. В аудит НЕ пишется полный ключ/тело/хэш/соль. Проверка: тест — для create и
  delete присутствует запись с timestamp+id+fingerprint; grep по выводу аудита не находит полный
  ключ/тело/хэш/соль. (baseline §1 «аудит create/delete: timestamp+id+fingerprint, не тело»; §4
  «структурно, без секретов»; spec AC; plan Аудит)

## Память (baseline §1, §4)

- [ ] **SR-25. Время жизни plaintext-ключа минимизировано (best-effort).** `PlainKey` (полный ключ в
  открытом виде) генерируется, отдаётся для одноразового вывода и НЕ сохраняется в долгоживущих полях
  `Store`/`Record`/глобалах; не пишется в логи/файлы/temp/аргументы. Ограничение: детерминированное
  затирание в памяти Go невыполнимо (managed-память/GC, `CGO_ENABLED=0`) — фиксируется честно как
  best-effort, не как гарантия (см. threat-model ОР-1, эскалация). Проверка: инспекция — `PlainKey`
  не оседает в полях структур хранилища; SR-13/SR-14/SR-21 подтверждают, что полный ключ не уходит в
  лог/arg/temp. (baseline §1, §4)
