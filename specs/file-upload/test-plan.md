...
- **Install-flow** — не применим к задаче `file-upload`. Фича не вводит новых install-артефактов
  (скриптов, бинарей, systemd-юнитов). Install-flow (`curl | sh`, детект uname/арки, SHA256)
  тестируется в задаче `distribution`. Данный test-plan его не охватывает.
...
| AC14 | цель-каталог → deny; I/O-ошибка (диск полон) → fail (isError:true, FAIL в аудите, сервер жив); невалидный mode → deny | unit+integration+**QA-добавлен** | `upload_test.go::TestWriteTargetIsDirectory`; `upload_tool_test.go::TestUploadFile_TargetIsDirectory/NoRequiredFields/SetuidDenied/WorldWritableDenied`; **`upload_qa_test.go::TestUploadFile_FailBranchIOError_MCP`** (ENOTDIR → fail, FAIL в аудите, сервер жив); **`::TestUploadFile_WriteAuditFailBranch_Unit`** (writeAudit Result:"fail"+isUpload прямо) | green |
...
**Итог по AC: все 20 AC покрыты. Пробелы по AC4(симлинк)/AC12(fp+remote)/AC14(fail-ветка)/AC16(413)/AC17/AC18 закрыты QA-тестами.**
...
| I/O-fail (ENOTDIR) → isError:true, FAIL в аудите, сервер жив | **`TestUploadFile_FailBranchIOError_MCP`** (файл вместо каталога в upload root) | AC14 | **QA добавил** |
| writeAudit fail+isUpload → FAIL tool=upload_file path= fp= remote= | **`TestUploadFile_WriteAuditFailBranch_Unit`** (writeAudit напрямую) | AC14/SR-78/SR-79 | **QA добавил** |
...
- **`TestUploadFile_FailBranchIOError_MCP`** — AC14/F-1: fail-ветка через MCP. Вектор I/O-fail:
  создать файл с именем промежуточного каталога (`notadir`) в upload root, запросить
  upload_file с `path="notadir/target.txt"`. `root.MkdirAll("notadir")` получает ENOTDIR
  (файл существует, а не каталог) — неизвестная ошибка → `auditResult="fail"`.
  Проверяет: isError:true; FAIL tool=upload_file в аудите; целевой файл не создан; сервер жив.
  Надёжен в Docker от root: ENOTDIR не зависит от euid.

- **`TestUploadFile_WriteAuditFailBranch_Unit`** — AC14/SR-78/SR-79/F-1: unit-тест writeAudit
  напрямую для `AuditRecord{Result:"fail", Tool:"upload_file", Path:"some/path.txt"}`.
  Гарантирует рендер: msg=FAIL, tool=upload_file, path=, fp=, remote=.
  Дополняет QA-7a: работает независимо от ОС и euid.
...
Дата: 2026-05-22 (обновлён после F-1/F-3 исправлений). Все пакеты зелёные.
...
ok  github.com/vladimirvkhs/raxd/internal/cli          0.048s
ok  github.com/vladimirvkhs/raxd/internal/cmdexec      1.180s
ok  github.com/vladimirvkhs/raxd/internal/config       0.005s
ok  github.com/vladimirvkhs/raxd/internal/fileupload   0.019s
ok  github.com/vladimirvkhs/raxd/internal/keystore     0.094s
ok  github.com/vladimirvkhs/raxd/internal/mcp          2.182s
ok  github.com/vladimirvkhs/raxd/internal/server       2.201s
...
ok  github.com/vladimirvkhs/raxd/internal/cmdexec      2.184s
ok  github.com/vladimirvkhs/raxd/internal/keystore     1.174s
ok  github.com/vladimirvkhs/raxd/internal/server       3.818s
ok  github.com/vladimirvkhs/raxd/internal/mcp          6.321s
```

Ключевые новые QA-тесты (из Docker, euid==0 от root):
```
...
=== RUN   TestUploadFile_AuditDenyHasFpAndRemote
--- PASS: TestUploadFile_AuditDenyHasFpAndRemote (0.00s)
...
=== RUN   TestUploadFile_AtomicityNoTempAfterWriteErrTooLarge
--- PASS: TestUploadFile_AtomicityNoTempAfterWriteErrTooLarge (0.00s)

=== RUN   TestUploadFile_FailBranchIOError_MCP
--- PASS: TestUploadFile_FailBranchIOError_MCP (0.00s)

=== RUN   TestUploadFile_WriteAuditFailBranch_Unit
--- PASS: TestUploadFile_WriteAuditFailBranch_Unit (0.00s)
```

AC20: ВЕРИФИЦИРОВАН. Все тесты зелёные в Docker, `-mod=vendor`, race чист. FAIL пакетов нет.
...
7. **AC14 (fail-ветка — I/O ошибка)** — `TestUploadFile_TargetIsDirectory` покрывал
   только ErrIsDir (deny), не сырую I/O ошибку (fail). Fail-ветка handler'а
   (`auditResult="fail"`) не имела реального теста. Добавлены:
   `TestUploadFile_FailBranchIOError_MCP` (ENOTDIR через MCP) и
   `TestUploadFile_WriteAuditFailBranch_Unit` (writeAudit напрямую).
