...
| AC6 | euid демона != 0 | unit + integration | `service/templates_test.go` (User=raxd в unit); integration STEP 2 (systemctl show User=raxd + LIVE euid) | PASS |
...
**Прогон: 2026-05-22, Docker Desktop macOS (cgroup v2)**
...
PASS: AC6/SR-83: LIVE — euid процесса демона != 0 (euid=999)
  [Примечание: raxd serve падает без config/TLS — ожидаемо в пустом контейнере]
...
STEP 4: restart-on-failure AC4
  INFO: PID поймали: 12614; kill -9 отправлен
  INFO: NRestarts=3 после kill
PASS: AC4: NRestarts=3 > 0 — демон перезапускался при сбое (auto-restart работает)
...
**Прогон: 2026-05-22, Docker (`Dockerfile` target test, golang:1.25)**
...
| AC6 | PASS | unit (User=raxd в unit) + integration (LIVE euid=999) |
