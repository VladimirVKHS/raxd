...
| AC6 | euid демона != 0 | unit + integration | `service/templates_test.go` (User=raxd в unit); integration STEP 2 (systemctl show User=raxd + LIVE euid=999 из /proc/$PID/status); STEP 4 (euid=999 после kill) | PASS |
...
**Прогон: 2026-05-22 (BUG-1 закрыт, ConfigurationDirectory в unit), Docker Desktop macOS (cgroup v2)**

Образ: `raxd-systemd-test` (ubuntu:22.04 + systemd). Контейнер: `raxd-svc-test` (privileged,
`--cgroupns=host`). Бинарь: `raxd_linux_amd64` (CGO_ENABLED=0, -mod=vendor, коммит 70ff715+).
...
INFO: Состояние сервиса после start: active   ← сервис держится (ConfigurationDirectory=raxd)
  [journald: cert generated /var/lib/raxd/tls/cert.pem; key generated /var/lib/raxd/tls/key.pem (0600)]
INFO: systemctl show User: raxd
PASS: AC6/SR-83: systemctl show User=raxd (сервис запускается под raxd, не root)
INFO: Result=success ActiveState=active
INFO: Текущий MainPID: 179
PASS: AC6/SR-83: LIVE — euid процесса демона != 0 (euid=999)   ← BUG-1 ЗАКРЫТ

STEP 3: status
PASS: AC1: status exit 0
PASS: AC1: status содержит 'installed'
PASS: SR-95: status без секретов rax_live_
  [stdout: installed=yes, running=yes, pid=179, euid=999, user=raxd [not root],
           port=7822, autostart=enabled, state=active (running)]

STEP 4: restart-on-failure AC4
  INFO: Попытка 1: MainPID=179, EUID=999  ← живой PID, сразу поймали
  INFO: kill -9 179 для теста AC4...
  INFO: PID после kill: 245 (рестарты: 1)
PASS: AC4: LIVE — restart-on-failure сработал (PID 179 → 245)
PASS: AC6/SR-83: euid после рестарта != 0 (euid=999)
PASS: AC6/SR-83: LIVE-захват EUID при старте демона = 999 (не root)
INFO: AC6/SR-83: LIVE euid верифицирован (флаг установлен)
...
══════════════════════════════════════════
ИТОГИ: PASS: 62  FAIL: 0
══════════════════════════════════════════
```

### Ограничения среды (честная фиксация)

1. **raxd serve требует ConfigurationDirectory** — `ConfigurationDirectory=raxd` в unit (827d736)
   обеспечивает создание `/etc/raxd` до старта демона. Без этого `raxd serve` немедленно падал,
   что не давало поймать MainPID>0. После фикса сервис stable-active: cert генерируется в
   `/var/lib/raxd/tls/`, LIVE euid=999 читается из `/proc/$PID/status`.

2. **StateDirectory в контейнере** — `/var/lib/raxd` создаётся systemd при старте сервиса.
   `StateDirectoryMode=0700` явно в unit (подтверждено unit-тестом + grep в integration).
...
4. **AC7 live-тест порта <1024** — требует конфига с `port: 443` и рабочего raxd serve
   с TLS. В пустом контейнере нет prod-config. Покрыто unit-тестами генератора (SR-85/86/87).
...
6. **BUG-1 (AC6/SR-83) — ЗАКРЫТ** (developer-коммиты 827d736/70ff715). Причина: без
   `ConfigurationDirectory=raxd` демон падал за <1s, PID-окно было нечитаемым. После фикса:
   сервис active, MainPID=179>0, LIVE euid=999 подтверждён в STEP 2 и STEP 4.

---

## Статус unit/race прогона

**Прогон: 2026-05-22 (переподтверждён после BUG-1 fix 827d736/70ff715), Docker (`Dockerfile` target test, golang:1.25)**
...
| AC6 | PASS | unit (User=raxd в unit) + integration: LIVE euid=999 из /proc/$PID/status в STEP 2 + STEP 4 (BUG-1 закрыт: ConfigurationDirectory в unit) |
